...
async function main() {
  for (const img of imgs) {
    ...
    const rings = result.targets[0]?.rings ?? [];
    if (!result.success || rings.length < 10) { console.log(`${img}: FAIL`); continue; }
    const r5 = splineRadius(rings[5]);
    const r7 = splineRadius(rings[7]);
