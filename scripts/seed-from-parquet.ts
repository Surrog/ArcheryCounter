...
function asyncBufferFromFile(filepath: string) {
  const buf = fs.readFileSync(filepath);
  return {
    byteLength: buf.byteLength,
    slice: (start: number, end?: number): Promise<ArrayBuffer> =>
      Promise.resolve(buf.buffer.slice(buf.byteOffset + start, buf.byteOffset + (end ?? buf.byteLength)) as ArrayBuffer),
  };
}
...
async function main() {
  ...
  const file = asyncBufferFromFile(PARQUET_PATH);
